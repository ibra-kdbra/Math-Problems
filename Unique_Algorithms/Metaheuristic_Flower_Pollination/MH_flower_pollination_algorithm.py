...
def six_hump_camel_back(variables_values=[0, 0]):
    func_value = (
        4 * variables_values[0] ** 2
        - 2.1 * variables_values[0] ** 4
        + (1 / 3) * variables_values[0] ** 6
        + variables_values[0] * variables_values[1]
        - 4 * variables_values[1] ** 2
        + 4 * variables_values[1] ** 4
    )
    return func_value
...
def rosenbrocks_valley(variables_values=[0, 0]):
    ...
    for i in range(1, len(variables_values)):
        func_value = (
            func_value
            + (100 * math.pow((variables_values[i] - math.pow(last_x, 2)), 2))
            + math.pow(1 - last_x, 2)
        )
    return func_value
