def main():
    ...
    parser = ArgumentParser(
        description="find prime implicants, essential prime implicants, and all minimum sum of products forms."
    )
    ...
    if len(sys.argv) > 1:
        parser.add_argument(
            ...
            type=int,
            choices=range(1, 27),
            help="Number of variables in the kmap",
        )
        ...
        num_of_variables = int(
            input("Enter the number of variables (Between 1 to 26): ")
        )
        bit_limit = (2**num_of_variables) - 1
        minterms = set(
            map(int, input(f"Enter the minterms (Between 0 to {bit_limit}): ").split())
        )
        dont_cares = set(
            map(
                int, input(f"Enter the dont cares (Between 0 to {bit_limit}): ").split()
            )
        )
    ...
    EPIs = list(
        map(my_kmap.translate_implicant, my_kmap.get_essential_prime_implicants())
    )
