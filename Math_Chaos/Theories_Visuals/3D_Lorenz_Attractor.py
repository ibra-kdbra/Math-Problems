...
from parameters import beta, rho, sigma
...
def gradient_line3d(ax, xs, ys, zs, cmap, alpha=1, linewidth=1):
    ...
    colors = cmap(norm(zs))
    segments = [
        np.array([[xs[i - 1], ys[i - 1]], [xs[i], ys[i]]]) for i in range(1, len(xs))
    ]
...
def main(inputs):
    ...
    soln = solve_ivp(lor, (0, 100), (0, 1, 1.05), args=(s, b, r), dense_output=True)
    ...
    fig.savefig("./assets/3d_lorenz.png")
    # plt.show()
