...
class Box:
    ...
    def DE(self, p):
        ...
        a = np.abs(p[:3] - c) - s
        return (
            min(max(a[0], a[1], a[2]), 0.0) + np.linalg.norm(np.maximum(a, 0.0))
        ) / p[3]
...
class InfCross:
    ...
    def DE(self, p):
        ...
        sq = (p[:3] - c) * (p[:3] - c)
        return (
            math.sqrt(min(min(sq[0] + sq[1], sq[0] + sq[2]), sq[1] + sq[2])) - r
        ) / p[3]
...
class InfCrossXY:
    ...
    def glsl(self):
        return (
            "de_inf_cross_xy(p" + cond_offset(self.c) + ", " + float_str(self.r) + ")"
        )
