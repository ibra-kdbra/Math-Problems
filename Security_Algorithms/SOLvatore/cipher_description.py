...
class CipherDescription:
    ...
    def apply_sbox(self, sbox_name, input_bits, output_bits):
        ...
        if len(self.sboxes[sbox_name]) != 2 ** len(input_bits):
            print(
                'Size of S-box "{}" incompatible with number of bits'
    ...
    def check_source(self, source):
        ...
        if source[0] == "s":
            ...
            if number >= self.state_size:
                raise ValueError(
                    "There are only {} state bits.".format(self.state_size)
                )
        elif source[0] == "t":
            if source not in self.temporaries:
                raise ValueError(
                    "There is no temporary bit named '{}' "
    ...
    def check_permutation(self, permutation):
        ...
        for var in permutation:
            if var[0] != "s":
                raise ValueError("Only 's' variables are allowed " "in permutations.")
            number = int(var[1:])
            if number >= self.state_size:
                raise ValueError(
                    "There are only {} state bits.".format(self.state_size)
    ...
    def apply_MC(self, wordsize, MC, Rp, nc, nr):
        ...
        for c in range(nc):
            for i in range(wordsize * nr):
                k = BM[i].index(1)
                self.apply_mov(
                    "t{}".format(nr * wordsize * c + k),
                    "s{}".format(nr * wordsize * c + i),
                )
                for j in range(k + 1, len(BM[i])):
                    if BM[i][j] == 1:
                        self.apply_xor(
                            "s{}".format(nr * wordsize * c + i),
                            "t{}".format(nr * wordsize * c + j),
                            "s{}".format(nr * wordsize * c + i),
                        )
        ...
        for q in range(d):
            for c in range(d):
                for i in range(wordsize):
                    k = BZ[i].index(1)
                    self.apply_mov(
                        "s{}".format(d * wordsize * c + k),
                        "t{}".format(wordsize * c + i),
                    )
                    for j in range(k + 1, len(BZ[i])):
                        if BZ[i][j] == 1:
                            self.apply_xor(
                                "t{}".format(wordsize * c + i),
                                "s{}".format(d * wordsize * c + j),
                                "t{}".format(wordsize * c + i),
                            )
            ...
            for i in range(d * wordsize):
                ...
                self.apply_mov(
                    "t{}".format(i),
                    "s{}".format((d - 1) * wordsize * (i / wordsize + 1) + i),
                )
    ...
    def shufflewords(self, shuffle, wordsize, rev):
        ...
        for i in range(len(shuffle)):
            ...
            while (
                cycles[len(cycles) - 1][0]
                != cycles[len(cycles) - 1][len(cycles[len(cycles) - 1]) - 1]
            ):
                cycles[len(cycles) - 1].append(
                    shuffle[cycles[len(cycles) - 1][len(cycles[len(cycles) - 1]) - 1]]
                )
    ...
    def add_mod(self, x, y, z, n, toffset):
        ...
        for i in range(1, n):
            ...
            self.apply_xor(
                "t{}".format(toffset + i),
                "t{}".format(toffset + i + n),
                "t{}".format(toffset + 2 * n + 1),
            )
            self.apply_and(
                "t{}".format(toffset + i),
                "t{}".format(toffset + i + n),
                "t{}".format(toffset + 2 * n + 2),
            )
            self.apply_and(
                "t{}".format(toffset + 2 * n + 1),
                "t{}".format(toffset + 2 * n),
