...
def generate_priv_key(bits, mode, *args):
    if bits > 1023 and (bits % 256) == 0:
        ...
        if mode == "file":
            ...
            if len(args) > 2 and args[1] == "DER":
                tag = configs["RSA_sc"]["BIN_TYPE"]
            else:
                tag = configs["RSA_sc"]["TEXT_TYPE"]
            with switch(len(args)) as s:
                if s.case(1):
                    open(path + name + tag, "+wb").write(key.export_key("PEM"))
                    return path + name + tag
                if s.case(2):
                    open(path + name + tag, "+wb").write(key.export_key(format=args[1]))
                    return path + name + tag
                if s.case(3):
                    open(path + name + tag, "+wb").write(
                        key.export_key(format=args[1], passphrase=args[2])
                    )
                    return path + name + tag
                if s.case(4):
                    open(path + name + tag, "+wb").write(
                        key.export_key(format=args[1], passphrase=args[2], pkcs=args[3])
                    )
                    return path + name + tag
                if s.case(6):
                    open(path + name + tag, "+wb").write(
...
def import_key(file, struct, password):
    ...
    if struct == "text":
        ...
        configs = configparser.ConfigParser()
        configs.read(
            os.path.dirname(os.path.dirname(__file__)) + "/Settings/" + "settings.ini"
        )
        if re.search(configs["RSA_sc"]["TEXT_TYPE"], name):
            return RSA.import_key(
                open(path + "/" + name, "rt").read(), passphrase=password
            )
        elif re.search(configs["RSA_sc"]["BIN_TYPE"], name):
            return RSA.import_key(
                open(path + "/" + name, "rb").read(), passphrase=password
            )
        else:
            return 255
...
def generate_pub_key(priv_key, mode, *args):
    if mode == "file":
        ...
        with switch(len(args)) as s:
            if s.case(1):
                open(path + name + tag, "+wb").write(
                    priv_key.publickey().export_key("PEM")
                )
                return path + name + tag
            if s.case(2):
                open(path + name + tag, "+wb").write(
                    priv_key.publickey().export_key(format=args[1])
                )
                return path + name + tag
            if s.case(3):
                open(path + name + tag, "+wb").write(
                    priv_key.publickey().export_key(format=args[1], passphrase=args[2])
                )
                return path + name + tag
            if s.case(4):
                open(path + name + tag, "+wb").write(
                    priv_key.publickey().export_key(
                        format=args[1], passphrase=args[2], pkcs=args[3]
                    )
                )
                return path + name + tag
            if s.case(6):
