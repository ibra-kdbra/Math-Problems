...
from Crypto_Brother.Algorithms import AES_sc, RSA_sc
...
def RSA_AL_key_alg(bits, priv_name, pub_name, only_pub):
    ...
    for false_keys in range(0, 5):
        ...
        files.append(
            RSA_sc.generate_pub_key(priv_key, "file", pub_name + str(false_keys))
        )
        if false_keys == randnum and only_pub == 1:
            priv_key = RSA_sc.import_key(priv_name, "text", "")
            files.append(
                RSA_sc.generate_pub_key(
                    priv_key, "file", pub_name + "appended" + str(false_keys)
                )
            )
        if false_keys == randnum and only_pub == 0:
            open(
                keysett.path
                + "/"
                + MD5_sc(bytes(os.path.basename(priv_name), "utf-8"))
                + ".pem.priv",
                "+wb",
            ).write(priv_key.export_key("PEM"))
    if only_pub:
        signature = RSA_sc.sign_keys(priv_name, "file", "text", "")
    else:
        signature = RSA_sc.sign_keys(
            keysett.path
            + "/"
            + MD5_sc(bytes(os.path.basename(priv_name), "utf-8"))
            + ".pem.priv",
    ...
    for key_file in files:
        ...
        if RSA_sc.verify_signature(key, signature):
            ...
            open(
                keysett.path + "/" + MD5_sc(bytes(pub_name, "utf-8")) + ".pub", "+wb"
            ).write(open(key_file, "rb").read())
            os.remove(key_file)
        else:
            os.remove(key_file)
...
def PKCS1_OAEP_decrypt(encrypted_file, decrypt_type, key):
    ...
    if os.path.dirname(encrypted_file) == "":
        path = filesett.path + "/"
    else:
        path = os.path.dirname(encrypted_file) + "/"
...
def AES_log_mode_encrypt(msg, msg_type, mode):
    ...
    if mode == 256 or mode == 128 or mode == 64:
        ...
        return 1  # Fail reason invalid mode or none one!
    ...
    if msg_type == "file":
        ...
        return 2  # Fail reason, invalid message type or non one
    ...
    open(path + "/" + MD5_sc(bytes(str(log), "utf-8")) + extension, "+wb").write(
        bytes(name, "utf-8")
    )
    ...
    open(logsett.path + "/" + logsett.name, "at").write(log)
    return (
        path + MD5_sc(bytes(str(log), "utf-8")) + extension,
...
def AES_log_mode_decrypt(msg, msg_type, raw_log):
        ...
        if part == 1:
            if re.search(ctime(os.path.getctime(path + "/" + old_name)), line):
                if re.search("starting AES", line):
                    if (
                        re.search(str(size - 8), line)
                        or re.search(str(size - 16), line)
                        or re.search(str(size - 32), line)
                    ):
                        if (
                            MD5_sc(bytes(line, "utf-8"))
                            + configs["AES_log_mode"]["MES_TYPE"]
                            == old_name
                            or MD5_sc(bytes(line, "utf-8"))
                            + configs["AES_log_mode"]["FILE_TYPE"]
                            == old_name
                        ):
                            print(2)
                            part = 2
    ...
    if key_file == "":
        return 2
    if msg_type == "file":
        open(path + "/" + name, "+wb").write(
            AES_sc.decrypt(open(keysett.path + "/" + key_file, "rb").read(), content)
        )
    if msg_type == "text":
        print(
            str(
                AES_sc.decrypt(
                    open(keysett.path + "/" + key_file, "rb").read(), content
                ),
                "utf-8",
            )
        )
